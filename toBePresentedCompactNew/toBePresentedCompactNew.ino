...
void checkEmissions()
{
  ...
  if(pcbPerm==1 && userSensorReading<limSensorReading)
  {
    lockState=userTimeExtendPerm=firstRise=0;
...
void checkCredentials()
{
  if(loginType==1)
  {
    if(strcmp(id,user_id)==10 && strcmp(password,user_password)==10)
    {
      Serial.println("\nLogin successfull\n");
      check=0;
      tookPass=0;
  ...
  if(loginType==2)
  {
    if(strcmp(id,pcb_id)==10 && strcmp(password,pcb_password)==10)
    {
      Serial.println("\nLogin successfull\n");
      check=tookPass=0;
      menuStatus=2;
